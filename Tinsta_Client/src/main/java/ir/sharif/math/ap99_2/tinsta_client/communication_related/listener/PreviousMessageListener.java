...
import ir.sharif.math.ap99_2.tinsta_shared.communication_related.event.PreviousMessageEvent;
...
public class PreviousMessageListener {
    Context context = new Context();

    public void eventOccurred(PreviousMessageEvent previousMessageEvent){
        ChatView chatView = (ChatView) previousMessageEvent.getSource();
    ...
    private void handleOfflineSavedMessage(ChatView chatView) {
        ...
        if (!chatRoom.hasPreviousSavedMessage()) {
            String msg = "Don't have Previous Message!";
            JOptionPane.showMessageDialog(chatView, msg,
                    "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        chatRoom.goPreviousSavedMessage();
    ...
    private void handleOfflineSingleChat(ChatView chatView) {
        ...
        if (!singleChat.hasPreviousMessage()) {
            String msg = "Don't have Previous Message!";
            JOptionPane.showMessageDialog(chatView, msg,
                    "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        singleChat.goPreviousMessage();
    ...
    private void handleOfflineGroup(ChatView chatView) {
        ...
        if (!group.hasPreviousMessage()) {
            String msg = "Don't have Previous Message!";
            JOptionPane.showMessageDialog(chatView, msg,
                    "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        group.goPreviousMessage();
        context.getChatRooms().update(chatRoom);
